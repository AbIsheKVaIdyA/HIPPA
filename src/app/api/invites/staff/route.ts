...
export async function POST(request: Request) {
  ...
  if (!isAdminRole(me.role as UserRole | undefined)) {
    await auditPhi(request, me, {
      action: "invite.staff",
      status: "failure",
  ...
  const nextPath = dashboardPathForRole(parsed.role as UserRole);
  ...
  const invitedId = data.user?.id ?? null;
  await auditPhi(request, me, {
    action: "invite.staff",
    status: "success",
    details: { target_role: parsed.role, invited_user_id: invitedId },
  });
  return NextResponse.json({ userId: invitedId });
}
